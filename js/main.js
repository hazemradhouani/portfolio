...
(function () {
  ...
  document.addEventListener('click', function (e) {
    ...
    /* Cross-page link that also has a hash, animate exit then navigate.
       The browser will handle the in-page scroll after load. */
    e.preventDefault();
    navigateTo(href);
  });
  ...
  (function () {
    ...
    /* ── About page: align button vertically with the Send button ──
       On the about page the Send button is a fixed visual landmark.
       We measure its vertical centre on load and on every scroll/resize
       and set the back-to-top button's top to match, so both sit on
       the exact same horizontal band regardless of viewport size.      */
    var submitBtn = document.querySelector('.contact-form__submit');
    if (submitBtn) {
      function alignWithSend() {
        var rect    = submitBtn.getBoundingClientRect();
        /* Vertical centre of the Send button in viewport coordinates */
        var centre  = rect.top + (rect.height / 2);
        /* Clamp so the button never goes off-screen */
        var clamped = Math.max(60, Math.min(window.innerHeight - 60, centre));
        /* Switch from bottom-based to top-based positioning */
        btn.style.bottom = 'auto';
        btn.style.top    = clamped + 'px';
      }
      alignWithSend();
      window.addEventListener('scroll', alignWithSend, { passive: true });
      window.addEventListener('resize', alignWithSend, { passive: true });
    }
