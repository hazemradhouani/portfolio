...
(function () {
  ...
  /* ─────────────────────────────────────────────────────────
     TOP-LEVEL PAGE SWIPE  (mobile)
     Swipe left → next page in sequence
     Swipe right → previous page in sequence
     Sequence: index → videos → about → books
     Only fires on top-level pages (not on project pages,
     which have their own swipe logic above).
  ───────────────────────────────────────────────────────── */
  (function () {
    /* Skip if this page already has project-level swipe */
    if (document.querySelector('.proj-nav__link--prev') ||
        document.querySelector('.proj-nav__link--next')) return;

    var pth = window.location.pathname;
    var currentIdx = -1;

    if (pth.endsWith('/') || pth.endsWith('index.html') || pth === '/') {
      currentIdx = 0;
    } else if (pth.indexOf('videos.html') !== -1) {
      currentIdx = 1;
    } else if (pth.indexOf('about.html') !== -1) {
      currentIdx = 2;
    } else if (pth.indexOf('books.html') !== -1) {
      currentIdx = 3;
    }

    if (currentIdx === -1) return;

    var PAGE_SEQ = ['index.html', 'videos.html', 'about.html', 'books.html'];
    var prevPage = currentIdx > 0              ? PAGE_SEQ[currentIdx - 1] : null;
    var nextPage = currentIdx < PAGE_SEQ.length - 1 ? PAGE_SEQ[currentIdx + 1] : null;
    if (!prevPage && !nextPage) return;

    var tStartX = 0, tStartY = 0, tMoveX = 0;
    var SWIPE_MIN   = 68;
    ...
    document.addEventListener('touchmove', function (e) {
      ...
      var moveY = e.touches[0].clientY - tStartY;

      if (Math.abs(tMoveX) > 8 || Math.abs(moveY) > 8) {
        if (Math.abs(tMoveX) > Math.abs(moveY) * SWIPE_RATIO) {
          isSwiping = true;
          isLocked  = true;
        } else {
          isLocked = true; /* vertical scroll wins */
        }
      }
      /* Subtle follow-finger pull, suppressed with reduced-motion */
      if (isSwiping && !prefersReducedMotion()) {
        var clamp = Math.max(-28, Math.min(28, tMoveX * 0.16));
        document.body.style.transform  = 'translateX(' + clamp + 'px)';
        document.body.style.transition = 'none';
      }
    }, { passive: true });
    ...
    document.addEventListener('touchend', function () {
      ...
      var dist  = tMoveX;
      isSwiping = false;
      if (dist < -SWIPE_MIN && nextPage) { navigateTo(nextPage, 'forward'); }
      else if (dist > SWIPE_MIN && prevPage) { navigateTo(prevPage, 'back'); }
    }, { passive: true });
    ...
    if (!prefersReducedMotion()) {
      ...
      for (var d = 0; d < PAGE_SEQ.length; d++) {
        var dot = document.createElement('span');
        dot.className = 'page-dot' + (d === currentIdx ? ' page-dot--active' : '');
  ...
  /* ─────────────────────────────────────────────────────────
     HERO NAV CLOSE BUTTON (mobile only)
     A rounded × button that appears below the hero nav links
     on the homepage. Tapping it smooth-scrolls to #works,
     giving users a clear "exit this menu" gesture.
  ...
  (function () {
    ...
    closeBtn.className = 'hero__close-btn';
    closeBtn.setAttribute('type',       'button');
    closeBtn.setAttribute('aria-label', 'Skip to works');
    closeBtn.innerHTML =
